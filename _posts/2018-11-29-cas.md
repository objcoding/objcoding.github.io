...
* content
{:toc}
在 Java 并发领域，我们解决并发安全问题最粗暴的方式就是使用 synchronized 关键字了，但它是一种独占形式的锁，属于悲观锁，性能会大打折。volatile 貌似也是一个不错的选择，但 volatile 只能保持变量的可见性，并不保证变量的原子性操作。
...
public class AtomicStampedReferenceTest {
  ...
  public static void main(String[] args) {
    ...
    new Thread(() -> {
      System.out.println("操作线程" + Thread.currentThread() + ",初始值 value = " + a.getReference());
      // 获取当前版本号
    ...
    new Thread(() -> {
      ...
      System.out.println("操作线程" + Thread.currentThread() + "value值:" + a.getReference());
      // 将 value 值又改成 1
...
```bash
操作线程Thread[线程1,5,main],初始值 value = 1
操作线程Thread[线程2,5,main]value值:2
操作线程Thread[线程2,5,main]value值:1
操作线程Thread[线程1,5,main],CAS操作结果: false
```
